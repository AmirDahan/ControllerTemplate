...
class Theme
{
    public static int ThemeMode = (int)Registry.GetValue(@"HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Themes\Personalize", "AppsUseLightTheme", null);
    ...
    public static void Read()
    {
        ...
        {
            MessageBox.Show("Unable to read Transparency" + Environment.NewLine + "Reverting to disabled", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning); //Show warning
        }
        else
        {
            Transparency = (int)Registry.GetValue(@"HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Themes\Personalize", "EnableTransparency", null);
        }
    }
    public static void Set(Control control) //This will check if your app contains any of the predefined colors and theme them accordingly
    {
        ...
        if (control.BackColor == Color.FromArgb(35, 35, 40)) { control.BackColor = BrightPanel; }
        if (control.HasChildren) { foreach (Control childControl in control.Controls) { Theme.Set(childControl); } }
    }

    //Acrylic
    public static void EnableAcrylic(Form window, Panel panel)
    {
        uint gradient;
        if (window is null) throw new ArgumentNullException(nameof(window));
        if (ThemeMode == 1) { Acryliccolor = Color.FromArgb(180, 255, 255, 255); }
        gradient = ToAbgr(Acryliccolor);
        Color panelcolor = Color.FromArgb(Math.Min(255, panel.BackColor.R - 1), Math.Min(255, panel.BackColor.G - 1), panel.BackColor.B); ;
